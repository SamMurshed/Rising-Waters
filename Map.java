...
public class Map {
	...
	public Map(double[][] elevations, GridPoint[] waterSources, double waterHeight) throws IllegalArgumentException {
		if (elevations == null || waterSources == null) {
			throw new IllegalArgumentException();
		}
		this.elevations = elevations;
		this.waterSources = waterSources;
		this.waterHeight = waterHeight;
		// So minElevation and maxElevation have values even though it is not used.
		this.maxElevation();
		this.minElevation();
	}
	...
	public double elevation(GridPoint p) throws NotOnMapException {
		...
		if (row < 0 || row >= elevations.length || column < 0 || column >= elevations[0].length) {
			throw new NotOnMapException("Grid point is not on map!");
		}
	 ...
	 * 
	 * @param p the grid point
	 * @return true if the grid point is below or at the specified height, false
	 *         otherwise
