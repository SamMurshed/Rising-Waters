...
public class Simulation {
	...
	public static void simulateRecursive(boolean visualize) {
		// Iterates through every water source.
		for (GridPoint source : map.waterSources()) {
			try {
				if (map.belowOrAtHeight(source)) {
					simRec(source);
				}
	...
	private static void simRec(GridPoint current) {
		// Recursion stops when the grid point is already underwater.
		if (underWater(current)) {
			return;
		} else {
			try {
				if (map.belowOrAtHeight(current)) {
					flooded[current.row()][current.col()] = true;
					if (current.row() < map.numOfRows() - 1) {
						simRec(current.down());
					}
					if (current.row() > 0) {
						simRec(current.up());
					}
					if (current.col() < map.numOfCols() - 1) {
						simRec(current.right());
					}
					if (current.col() > 0) {
						simRec(current.left());
					}
	...
	public static void simulateWithStack(boolean visualize) {
		...
		for (GridPoint source : map.waterSources()) {
			try {
				if (map.belowOrAtHeight(source)) {
					// Add adjacent gridpoints of water source to stack.
					flooded[source.row()][source.col()] = true;
					if (source.row() < map.numOfRows() - 1) {
						stack.push(source.down());
					}
					if (source.row() > 0) {
						stack.push(source.up());
					}
					if (source.col() < map.numOfCols() - 1) {
		...
		while (!stack.isEmpty()) {
			// Pop grid point from top of stack.
			GridPoint current = stack.pop();
			try {
				// If the grid point is not underwater and it is below or at height, advance to
				// nested if statements, else it was just popped off.
				if (!underWater(current) && map.belowOrAtHeight(current)) {
					flooded[current.row()][current.col()] = true;
					if (current.row() < map.numOfRows() - 1) {
	 ...
	 * 
	 * @param visualize true if the progress should be visualized, false otherwise
	 * @throws NotOnMapException
	 */
	public static void simulateWithQueue(boolean visualize) {
		// For each water source at or below water level.
		for (GridPoint source : map.waterSources()) {
			try {
				if (map.belowOrAtHeight(source)) {
					// Add adjacent gridpoints of water source to queue.
					flooded[source.row()][source.col()] = true;
					if (source.row() < map.numOfRows() - 1) {
						queue.add(source.down());
					}
					if (source.row() > 0) {
						queue.add(source.up());
					}
					if (source.col() < map.numOfCols() - 1) {
						queue.add(source.right());
					}
					if (source.col() > 0) {
						queue.add(source.left());
					}
				}
			} catch (NotOnMapException e) {
			}
		...
		while (!queue.isEmpty()) {
			// Pop grid point from queue
			GridPoint current = queue.remove();
			try {
				// If the grid point is not underwater and it is below or at height, advance to
				// nested if statements, else it was just dequeued.
				if (!underWater(current) && map.belowOrAtHeight(current)) {
					flooded[current.row()][current.col()] = true;
					if (current.row() < map.numOfRows() - 1) {
		...
		for (int i = 0; i < flooded.length; i++) {
			for (int j = 0; j < flooded[0].length; j++) {
				if (flooded[i][j]) {
					...
					try {
						System.out.print(String.format("%2.0f ", map.elevation(new GridPoint(i, j))));
					} catch (NotOnMapException e) {
					}
				}
			}
	...
	/**
	 * Visualizes the current state of the world. The visualization is done by
	 * printing un-flooded grid points with one decimal place and flooded grid
	 * points
	 * using "XXX" sequence.
	 */
		...
		for (int i = 0; i < flooded.length; i++) {
			for (int j = 0; j < flooded[0].length; j++) {
				if (flooded[i][j]) {
					...
					try {
						System.out.print(String.format(" %3.1f ", map.elevation(new GridPoint(i, j))));
					} catch (NotOnMapException e) {
					}
				}
			}
		...
		for (int i = 0; i < flooded.length; i++) {
			...
			for (int j = 0; j < flooded[0].length; j++) {
				if (flooded[i][j]) {
					...
					try {
						System.out.print(shade(map.elevation(new GridPoint(i, j)), lowPoint, quarterRange));
					} catch (NotOnMapException e) {
					}
				}
			}
